...
const sections = mapGetters('featuredStore', ['sections']);
const currentSection = ref('');

onMounted(async () => {
  // Load all the existing feature list.
  try {
    await dispatch('featuredStore/getAllFeaturedList');
    currentSection.value = sections.value.length > 0 ? sections.value[0] : '';
    console.log('Setting current section: ', currentSection.value);
  } catch (error) {
    console.log('Feature get error');
  }
});
